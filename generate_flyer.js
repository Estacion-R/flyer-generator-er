...
function socialPillHtml(name, fg) {
  const info = SOCIAL_ICONS[name];
  if (!info) return '';
  return `<div class="pill"><svg viewBox="0 0 24 24" style="width:22px;height:22px;flex-shrink:0;display:block;" fill="${fg}"><path d="${info.icon}"/></svg><span>${escapeHtml(info.handle)}</span></div>`;
}
